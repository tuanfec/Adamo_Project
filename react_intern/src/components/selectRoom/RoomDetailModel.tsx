    ...
    <div className="flex flex-col lg:flex-row gap-4 p-5 ">
      <div className="w-full md:h-1/2 lg:w-1/2">
        <div className="flex flex-col gap-2 mb-4">
          <p className="text-2xl font-bold dark:text-white">{data.name}</p>
          <div className="flex justify-between items-center gap-2">
            <div className="flex items-center gap-2">
              {data.discount && (
                <span className="text-md text-[#5E6D77] dark:text-[#bbbbbb] mt-1 font-normal line-through">
                  ${data.price - (data.price * data.discount) / 100}
                </span>
              )}
              <p className="text-[#5E6D77] dark:text-[#bbbbbb] text-sm">
      ...
      <div className="w-full  lg:w-1/2 lg:mx-10 lg:my-10">
        <div className="flex justify-between mt-2 w-full">
          ...
          <div className="flex items-center gap-2">
            <FaBed className="text-[#4F4F4F] dark:text-[#e8e7e7] text-xl" />
            <span className="text-[#0069E4] dark:text-[#bbbbbb] text-md font-medium">
              {data.bed}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <BiShapeSquare className="text-[#4F4F4F] dark:text-[#e8e7e7] text-xl" />
            <span className="text-[#0069E4] text-md dark:text-[#bbbbbb] font-medium">
              {data.area} m2
            </span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {data.amenities.map((item) => (
              <div className="flex items-center gap-2">
                ...
                <p className="text-[#1E1E1ECC] dark:text-[#bbbbbb]">{item}</p>
              </div>
            ))}
          </div>
        </div>
